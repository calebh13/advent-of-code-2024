...
int main(void)
{
	...
	for (int lineNum = 0; getline(infile, line); lineNum++)
	{
		...
		for (int i = 0; i < nums.size(); i++)
		{
			...
			{
				// For each required page, if we haven't seen it, add it to illegalNums;
				// the required pages can't appear now, otherwise list is invalid
		...
		if (!correct)
		{
			// now we need to re-order nums.
			std::sort(nums.begin(), nums.end(), [&rules](int a, int b) 
			{
				// We need to return whether a < b
				auto iter = rules.find(b);
				// If b is in the list, and a is a "dependency" of b, a < b is true
				if (iter != rules.end() && iter->second.find(a) != iter->second.end())
				{
					return true;
				}
				else return false;
